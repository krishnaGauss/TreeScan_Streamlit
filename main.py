        ...
        with st.spinner("Generating..."):
            ...
            if mode == "QR Generation":
                ...
                if n_pdf_only > n_xlsx_only:
                    ...
                    expander_label = (
                        "Summary — All records matched"
                        if n_pdf_only == 0
                        else f"Summary — {n_pdf_only} mismatch(es)"
                    )
                    with st.expander(expander_label):
                        if n_pdf_only == 0:
                            st.write("All PDFs and records were matched successfully.")
                        else:
                            st.markdown("**PDFs with no matching record:**")
                            st.markdown("\n".join(f"- {name}.pdf" for name in pdf_not_in_xlsx))
                            st.markdown("**Records with no matching PDF:**")
                            st.markdown("\n".join(f"- {s}" for s in xlsx_not_in_pdf))
